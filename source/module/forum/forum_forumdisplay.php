...
if($_G['forum']['type'] == 'forum') {
	...
	if(empty($_GET['archiveid'])) {
		...
		$navigation = ' <em>&rsaquo;</em> <a href="forum.php?gid='.$forum_top['fid'].'">'.$forum_top['name'].'</a><em>&rsaquo;</em> <a href="forum.php?mod=forumdisplay&fid='.$forum_up['fid'].'">'.$forum_up['name'].'</a><em>&rsaquo;</em><a href="forum.php?mod=forumdisplay&fid='.$_G['forum']['fid'].'"> '.$_G['forum']['name'].'</a>';
...
$count=0;
$online=0;
$capplyjoin=DB::query("select * from ".DB::table('common_admincp_applyjoin')."  where fid=".$_G['fid'].' and te=1');
while($value = DB::fetch($capplyjoin)) {
		$applyjoinid = $value['id'];
		$app_uid = $value['uid'];
		$app_online=DB::query("select * from ".DB::table('common_session')."  where uid=".$app_uid);
		if($app_online){
			while($values = DB::fetch($app_online)) {
					$online +=1;
			}
		}
		$count+=count($applyjoinid);
	}
	
$napplyjoin=DB::query("select uid,te from ".DB::table('common_admincp_applyjoin')."  where fid=".$_G['fid'].' and uid='.$_G['uid']);
while($value = DB::fetch($napplyjoin)) {
	$applyjoinuid = $value['uid'];
	$applyjointe = $value['te'];
}


$moderator=DB::query("select uid,te from ".DB::table('common_admincp_moderator')."  where fid=".$_G['fid'].' and te=1');
while($value = DB::fetch($moderator)) {
	$moderatorid = $value['uid'];
}
