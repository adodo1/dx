...
if($op == 'edit') {
	...
	if(!$_GET['username'] && $_GET['fid']){
		
		$modcpsql = DB::query("select * from  ".DB::table('common_admincp_applyjoin')." where te=1 and fid=".$_GET['fid']);
		$modcp_print=array();
		$i=0;
		while($type = DB::fetch($modcpsql)) {
			
			$modcp_print[$i]["id"] = $type["id"];
			$modcp_print[$i]["fid"] = $type["fid"];
			$modcp_print[$i]["username"] = $type["username"];
			$modcp_print[$i]["sectionname"] = $type["sectionname"];
			$modcp_print[$i]["time"] = $type["time"];
			$modcp_print[$i]["uid"] = $type["uid"];
			$modcp_print[$i]["integral"] = $type["integral"];
			$modcp_print[$i]["i"] = $i;
			$i=$i+1;
		}
	}
